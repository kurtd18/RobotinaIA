"""
Igual que backtest_short_ada_4h.py, pero sobre las 9 criptos que
veníamos siguiendo desde el principio de la sesión (HYPE y XMR quedan
fuera, no están disponibles en Binance spot).

Misma señal (run-up sobre percentil 75 propio de cada activo, en velas
de 4h) y misma salida (trailing stop invertido para short).

No modifica la base de datos ni envía nada a Telegram - es de solo lectura.

Uso:
    python scripts/backtest_short_cripto_4h.py
"""
...
def correr():
    print(f"Estrategia SHORT: {len(CRIPTOS)} criptos, velas de 4h, desde {FECHA_INICIO.strftime('%Y-%m-%d')}")
    print(f"Señal: run-up sobre el percentil 75 propio de cada activo")
    ...
    for symbol in CRIPTOS:
        ...
        print(f"{symbol:10} umbral={umbral_runup:5.2f}% | {len(operaciones):3} operaciones | "
